...
def main():
    ...
    nodes = {}       # key -> (x, y, path tuple of action idxs)
    q = deque()
    for s in seeds:
    ...
    while q and len(nodes) < args.max_nodes and len(paths) < args.max_paths:
        ...
        for a in range(env.num_actions):
            ...
            if flvl >= goal:
                p = path + (a,)
                paths.append((p, (x, y), (fx, fy)))
                print(f"PATH ({len(p)} actions) -> lvl {flvl} ({fx},{fy}):")
                node, trail = None, []
                # reconstruct the visited spots along the path
                cx, cy = None, None
                env2 = None  # replay below for readability
    ...
    if not paths:
        ...
        # Replay the shortest path and bank each intermediate settled state as
        # a curriculum start, scored by progress along the path.
        p = min(paths, key=lambda t: len(t[0]))[0]
        seed = min(paths, key=lambda t: len(t[0]))[1]
        env.teleport(lvl, seed[0], seed[1])
